...
module.exports = function(app) {
  ...
  async function autoupdateAllEntity(){
    ...
    //printFoundedModel();
    var mariaDb = app.dataSources.mariaDb;

    for (var model in app.models) {
        if(model!='User') {
          try {
              await mariaDb.autoupdate(model);   
          }catch(error){
  ...
  async function initializateServices(){
    ...
    // Valutare se spostare in file di configuarzione.
    var secret_key = "aaaaaaaaa"; 
    ...
    if(await Factory.startExternalServices()){
       console.log('Factory service correctly started');
    }else
    {
       console.log('ERROR: Factory service stopped');
    }

    if(await Services.start(secret_key)){
