...
angular.module('xenon.controllers', []).
	...
	controller('MainCtrl', function($scope,$state, $rootScope, $location, $layout, $layoutToggles, $pageLoadingBar, Fullscreen,Users)
	{
	...
	controller('SidebarMenuCtrl', function($scope, $rootScope, $menuItems, $timeout, $location, $state, $layout,AuthService,Users,$urlRouter,ASSETS,Pddstorageb)
	{
		...
		/*$rootScope.$on("$stateChangeStart", function handleStateChangeStart(evt, toState) {
		        ...
		        console.log("toState",toState.data)
		        $rootScope.pagetitle = toState.data.title;
				
				// per cambiare il titolo pagine 
		...
		Users.getCurrent().$promise.then(function(response) {			
	        ...
	        Users.findWithFoto({parameter:'users',id:userid,companyids:null}).$promise.then(function(user) { 
	        	    

	        		$rootScope.user = user[0]	 // nome utente in header          

			        var obj =  JSON.parse(self.currentUser.activeoperations);

			       

			          // Menu Items
					var $sidebarMenuItems = $menuItems.instantiate(); // creo il menù in sidebar 					

					$scope.menuItems = $sidebarMenuItems.prepareSidebarMenu(obj).getAll();								
					...
					reloadMenu(self.currentUser)  // creo gli state dinamici
    		...
    		AuthService.getCurrentUser(function(currentUser){ 
		        ...
		        if(currentUser.roleids == '1'){ // admin 
		         ...
		         /*Alarm.find_({companyId:companyId,installationcompanyId:installationcompanyId}).$promise.then(function(alarms) { 
                        ...
                        $rootScope.alarmsnav = alarms.results
        ...
        $rootScope.goToAlarms = function(){

        	 $state.go('app.allarmi-visualizzaallarmi');
        }



		function reloadMenu(user){
	        const activeoperations = JSON.parse(user.activeoperations);
	          
	        var states;
	          for(var i=0; i<activeoperations.length; i++){

	          	 

	              var active = activeoperations[i];

	              var category = activeoperations[i].category.name.toLowerCase()

	              for(var j=0; j<active.operations.length; j++){


	                  var operations = active.operations[j];



	                const controller = 'js/controllers/'+category+'/' + operations.controllerjs;                     
	                
	                let plugins = '';
	                if(operations.dependencies){
	                  plugins = [operations.dependencies];  
	                }
	                
	                const plugins_ = plugins;

	                var state =   { 
	                                url: "/"+category+'-'+ operations.slug, 
	                                templateUrl: "views/"+category+'/' + operations.view,
	                                controller: operations.controllername,
	                                params: {
	                                  operations: null
	                                },
	                                data: {
	                               ...
	                               resolve: {
	                                  ...
	                                  loadPlugins: function($ocLazyLoad) {
	                                    return $ocLazyLoad.load(controller);
	                                  }
	                                }   
	                ...
	                var stateName = 'app.'+category+'-'+operations.slug;

	               
	                try {
	                  var retState = $state.get(stateName);
	                 
	                  if (!retState) {
	                    $stateProviderRef.state(stateName, state);
	                   
	                  }                    
	                } catch(e) {
	                  console.log('$stateProviderRef.state error:', e.message);
	                }
	            }   
	        }
	        ...
	        user.activeoperations = JSON.stringify(activeoperations);
